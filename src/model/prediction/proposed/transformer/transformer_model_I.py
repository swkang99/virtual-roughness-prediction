...
class ModalityAwareTokenEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # These are not used for training loss.
        with torch.no_grad():
...
class TransformerRegressor(nn.Module):  # Total trainable parameters ~= 109,188
    """
    Gated Hierarchical Local-Global Roughness Token Transformer.
    ...
    Important:
        ...
        Modality relevance is learned by gates:
            texture gate
            height gate
            normal gate
    ...
    def forward(self, height_img1, height_img2, height_img3):
        ...
        # [B, 5, 256, 256]
        # -> modality-aware token embedding
        # -> local roughness token aggregation
        # -> [B, 256, D]
        window_features, grid_h, grid_w = self._encode_local_windows(x)

        # [B, 256, D]
        # -> global roughness token aggregation
        # -> [B, D]
