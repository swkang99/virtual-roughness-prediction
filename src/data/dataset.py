...
class OriginalDataset(Dataset): 
    def __init__(self, df, conf, target_col):
        ...
        # Do NOT resize/crop images here.
        # The model is designed to receive the original 256 x 256 inputs.
        # If a non-256 image is found, fail fast instead of silently changing resolution.
        self.expected_size = (256, 256)  # PIL Image.size uses (width, height)
    ...
    def _check_image_size(self, image, path, image_name):
        if image.size != self.expected_size:
            raise ValueError(
                f"Expected {image_name} size {self.expected_size}, "
    ...
    def __getitem__(self, idx):
        ...
        self._check_image_size(texture_image, texture_path, "texture image")
        ...
        if self.conf['dataset_input'] == 'texture_maps':
            ...
            self._check_image_size(height_map, height_path, "height map")
            height_map = self.transform(height_map)

            normal_path = row["normal_path"]
            normal_map = Image.open(normal_path).convert("RGB")
            self._check_image_size(normal_map, normal_path, "normal map")
