...
class LearnableGlobalRoughnessPool(nn.Module):
    """
    Learnable spatial attention pooling for global roughness aggregation.
    ...
    Meaning:
        The module learns which spatial window features are important
        for final roughness prediction.
    """
    ...
    def forward(self, x):
        ...
        # [B, 1, H, W] -> [B, 1, H*W]
        scores = self.score_net(x).flatten(2)

        # Learn spatial importance over H*W locations.
        # weights: [B, 1, H*W]
        ...
        # [B, C, H*W] * [B, 1, H*W] -> [B, C]
        pooled = (x_flat * weights).sum(dim=-1)

        return pooled


class TransformerRegressor(nn.Module):  # Total trainable parameters = 111,042
    """
    Texture gray + height gray + normal local-window Transformer-based roughness regressor
    with learnable local roughness token and learnable global spatial attention pooling.
    ...
    def __init__(
        ...
        # Learnable global roughness pooling.
        # CNN head output channel is embed_dim // 2.
        self.global_pool = LearnableGlobalRoughnessPool(embed_dim // 2)
